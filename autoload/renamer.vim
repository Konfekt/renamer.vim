function! renamer#StartRenamer(needNewWindow, startLine, ...) "{{{1
...
function! <SID>PerformRename() "{{{1
  ...
  exec 'call renamer#StartRenamer(0,-1,b:renamerDirectory)'
...
function! <SID>ChangeDirectory() "{{{1
  ...
  exec 'call renamer#StartRenamer(0,lineForNewBuffer,b:renamerDirectory)'
endfunction
...
function! <SID>DeleteEntry() "{{{1
  ...
  if ok == '' || ok =~ '[yY]'
    ...
    exec 'call renamer#StartRenamer(0,lineNum,b:renamerDirectory)'
